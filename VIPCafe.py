...
#Code
def vip_cafe(n, priorities, k):
    orders = [(priority, i) for i, priority in enumerate(priorities)]

    orders.sort(reverse=True)

    for i, (priority, original_position) in enumerate(orders):
        if original_position == k - 1:
            return i + 1
